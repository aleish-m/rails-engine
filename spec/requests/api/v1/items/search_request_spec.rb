  ...
  describe 'search index' do
    ...
    describe 'sad path' do
      it 'returns a successful status code and a empty item array when no items are found on name search' do
        ...
        expect(items).to have_key(:data)
        expect(items[:data]).to be_a(Array)
        expect(items[:data].count).to be(0)
      end
      ...
      it 'returns a 400 status code and a item error array when sent a negitive price paramater' do
        item_1 = create(:item)
        item_2 = create(:item)
        item_3 = create(:item)
        item_4 = create(:item)

        get '/api/v1/items/find_all?min_price=-5&max_price=250'
